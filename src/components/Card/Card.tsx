...
export const Card = React.memo((props: Props) => {
  ...
  const anime =
    (item as AnimeType).title !== undefined ? (item as AnimeType) : null;
  const character =
    (item as CharacterType).name !== undefined ? (item as CharacterType) : null;
  ...
  return (
    <article className={styles["card"]}>
      ...
      {character && role ? (
        <h4 className={styles["card__label"]}>{role}</h4>
      ) : null}
